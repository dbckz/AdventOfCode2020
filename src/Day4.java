...
public class Day4 {
    ...
    private List<Map<String, String>> calculate(List<Map<String, String>> input) {
        int numValid = 0;
        int numInvalid = 0;
        int numNPC = 0;
        List<Map<String, String>> validPassports = new ArrayList<>();

        for (Map<String, String> doc : input) {
            if (doc.containsKey("byr") &&
                    doc.containsKey("iyr") &&
                    doc.containsKey("eyr") &&
                    doc.containsKey("hgt") &&
                    doc.containsKey("hcl") &&
        ...
        System.out.println("NPC: " + numNPC);
        return validPassports;
    }

    private List<Map<String, String>> calculate2(List<Map<String, String>> input) {
        List<Map<String, String>> valid = new ArrayList<>();

        for (Map<String, String> doc : input) {
            int byr = Integer.parseInt(doc.get("byr"));
            if (!(byr >= 1920 && byr <= 2002)) {
                continue;
            }

            int iyr = Integer.parseInt(doc.get("iyr"));
            if (!(iyr >= 2010 && iyr <= 2020)) {
                continue;
            }

            int eyr = Integer.parseInt(doc.get("eyr"));
            if (!(eyr >= 2020 && eyr <= 2030)) {
                continue;
            }

            String hgt = doc.get("hgt");
            if (hgt.endsWith("cm")) {
...
//        String input = "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd byr:1937 iyr:2017 cid:147 hgt:183cm,iyr:2013 ecl:amb cid:350 eyr:2023 pid:028048884 hcl:#cfa07d byr:1929,hcl:#ae17e1 iyr:2013 eyr:2024 ecl:brn pid:760753108 byr:1931 hgt:179cm,hcl:#cfa07d eyr:2025 pid:166559648 iyr:2011 ecl:brn hgt:59in";
        ...
        System.out.println("Total docs: " + inputList.size());
        List<Map<String, String>> documents = new ArrayList<>(inputList.size());
        ...
        Day4 day4 = new Day4();
        List<Map<String, String>> valid = day4.calculate(documents);
        System.out.println("Answer: " + valid.size());
        List<Map<String, String>> valid2 = day4.calculate2(valid);
        System.out.println("Answer part 2: " + valid2.size());
